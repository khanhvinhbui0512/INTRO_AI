...
def minimax(prev_board, board, eval, player, depth, eval_parent):
    if (depth <= 0 or eval == -16*player): return eval, ((0, 0), (0, 0))
    res = -20 * player
    pos = ((0, 0), (0, 0))
    if prev_board != None:
        pos1 = ()
                ...
                if (board[index[0]][index[1]] == player):
                    ...
                    if(player == 1):
                        if(temp_eval > eval):
                            ...
                            tmp_res, tmp_pos = minimax(board, new_board, temp_eval, -player, temp_depth-0.75,20 * player)
                            if ( tmp_res >  res):
                                res = tmp_res
                ...
                if (board[i][j] == player):
                    for index in available_move[i][j]:
                        if board[index[0]][index[1]] == 0:
                            ...
                            temp_depth = copy.deepcopy(depth)
                            tmp_res, tmp_pos = minimax(board,new_board, calc, -player, temp_depth - 1, res)
...
def move(prev_board, board, player, remain_time_x, remain_time_o):
    ...
    res, move = minimax(prev_board, board, eval, player,5 , 20 * player)
    return move
